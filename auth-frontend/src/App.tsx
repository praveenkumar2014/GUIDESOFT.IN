...
function courseBlurb(categoryId: string) {
  const blurbs: Record<string, string> = {
    'ai--generative-ai': 'Move from model concepts to prompts, evaluation, and a workflow you can use.',
    'ux-ui--product-design': 'Learn the decisions behind clear interfaces, from user needs to useful prototypes.',
    'full-stack-development': 'Connect frontend, backend, data, and deployment into one working product.',
    'frontend-development': 'Build responsive interfaces with a sharper eye for structure, interaction, and detail.',
    'python--backend': 'Write readable scripts, services, and automations that solve a real problem.',
    'cloud-computing': 'Understand the moving parts behind reliable, deployable digital products.',
    'career--industry-programs': 'A focused professional path with a practical outcome and a clear place to begin.',
  }
  return blurbs[categoryId] ?? 'A practical path with clear concepts, guided practice, and a project to finish.'
}
...
function DiscoveryCourseCard({ course, onNavigate, index = 0 }: { course: CatalogCourse; onNavigate: (path: string) => void; index?: number }) {
  return (
    <article className={`discovery-course-card discovery-course-card-${courseAccent(course)}`}>
      <PosterArt course={course} index={index + 1} accent={courseAccent(course)} compact />
      <div className="discovery-course-body"><p>{course.category}</p><h3>{course.title}</h3><p className="discovery-course-copy">{courseBlurb(course.categoryId)}</p><div className="discovery-course-meta"><span><Icon name="clock" size={13} /> Self-paced</span><span><Icon name="layers" size={13} /> 4-part path</span></div><button type="button" onClick={() => onNavigate(`/learn?course=${encodeURIComponent(course.id)}`)}>Open course <Icon name="arrow-up-right" size={14} /></button></div>
    </article>
  )
}
